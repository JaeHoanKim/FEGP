...
sample.PTESS = function(X, Z, kappa.pr = function(x){return(1)}, Nk, Tk, N.pr,
                               beta = 2, mcmc, brn, thin, sigsq = 0.01, kappa.init = 2,
                               g.init = rep(0, N.init+1)){
   ## X, Y: given data
   ## N.pr: prior distribution of N (function)
   ...
   if(dim(X)[1]!=length(Z))
      stop("X and Z should be of same length!")
   if(length(Tk) != length(Nk)){
      stop("Tk and Nk should have the same length!")
   }
   if (Tk[1] != 1){
   ...
   for(i in 1:em){
      ...
      for (kk in 1:(1*length(Nk))){
         ...
         log.prob.swap = min(0, (Tk[k2] - Tk[k1])/(Tk[k1] * Tk[k2]) *
                                (loglik2D(Z, X, g.out.mat[[k2]], sigsq) + log(N.pr(Nk[k2])) -
                                    loglik2D(Z, X, g.out.mat[[k1]], sigsq) - log(N.pr(Nk[k1]))))
         u = runif(1)
...
sample.PTexact = function(X, Z, kappa.pr = function(x){return(1)}, Nk, Tk, N.pr,
   ...
   for(i in 1:em){
      ...
      for (kk in 1:(1*length(Nk))){
         ...
         k2 = chains[2]
         log.prob.swap = min(0, (Tk[k2] - Tk[k1])/(Tk[k1] * Tk[k2]) *
                                (loglik2D(Z, X, g.out.mat[[k2]][[i]], sigsq) + log(N.pr(Nk[k2])) -
